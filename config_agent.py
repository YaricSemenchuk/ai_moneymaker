...
# Жёсткий circuit breaker: когда мягкого торможения мало — полностью
# выключаем агента на N часов. Снимается автоматически по истечении срока
# или вручную через дашборд (POST /api/agents/<id>/unpause).
HARD_PAUSE_BANS_24H = 10       # реальных банов за сутки → полная пауза
HARD_PAUSE_FAILS_24H = 40      # суммарных фейлов за сутки → полная пауза
HARD_PAUSE_HOURS = 24          # на сколько ставим паузу
