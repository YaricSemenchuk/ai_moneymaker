...
MESSAGE_DELAY_MAX = 300
MAX_MESSAGES_PER_HOUR = 8           # 5→8 после фиксов воронки
MAX_GROUPS_TO_JOIN_PER_DAY = 12
MIN_GROUP_MEMBERS = 500             # повысили со 100 — мелкие группы дают мусорный трафик
MAX_GROUP_MEMBERS_FOR_PROACTIVE = 100000  # слишком крупные = низкий CTR, посты тонут

# Реактив (listening) — пороги
LISTENING_INTEREST_THRESHOLD = 0.20  # минимум для ответа (общий keyword-based)
LISTENING_EMOTIONAL_BONUS = 0.40     # +к interest если есть emotional trigger
REPLY_VIA_DM = True                  # отвечать в личку + короткий хук в группе
...
MAX_TOTAL_ACTIONS_PER_DAY = 30

# Проактивный режим (агент сам пишет первым)
PROACTIVE_ENABLED = True
PROACTIVE_MAX_POSTS_PER_DAY = 6      # 4→6 на аккаунт
PROACTIVE_MIN_INTERVAL_HOURS = 1.5
PROACTIVE_MAX_INTERVAL_HOURS = 4
...
PROACTIVE_ACTIVE_HOUR_START = 10
